...
async function LoadData(pageNumber=1,pageSize=2,searchText="")
{
    ...
    if(searchText==="")
    {
        ...
        `http://localhost:5199/api/Employees?searchText=${searchText}?pageNumber=${pageNumber}&pageSize=${pageSize}`;
    }
...
async function showEmployees(flag = false,employeeId = -1,pageNumber = 1, pageSize = 2,searchText="")
{
    ...
    for (let i in EmployeeList)
    {
        ...
        let htmlString=`<td>${employee.employeeId}</td>
                        ...
                        <td>
                        <button onclick="updateEmployee(true,${employee.employeeTd},${pageNumber},${pageSize},${searchText})">Update</button>
                        <button onclick="deleteEmployee(${employee.employeeTd},${pageNumber},${pageSize},${searchText})">Delete</button>
                        </td>`;
            ...
            if(employee.employeeId==employeeId && flag)
            {
                htmlString=`<td>${employee.employeeId}</td>
                           ...
                           <td><input type="number" id="updatedSalary" value="${employee.salary}"></td>
                            <td>
                                <button onclick="updateEmployee(false,${employee.employeeTd},${pageNumber},${pageSize},${searchText})">Update</button>
